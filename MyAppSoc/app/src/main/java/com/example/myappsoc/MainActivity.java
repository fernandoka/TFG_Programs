...
public class MainActivity extends AppCompatActivity implements
    ...
    protected SendDataBL sendDataBL;
    ...
    @Override
    public void blOnOffOnClick() {
        if(myBluetoohAdapter==null)
            myPrintMsg("This device dosen't support bluetooth");
        else{
            if(!myBluetoohAdapter.isEnabled())
                startActivityForResult(btEnablingIntent, MyConstants.REQ_CODE_ENGABLE_BL);
            else{
                appStatus.updateStatusBl(MyConstants.STATE_BL_OFF);
                myBluetoohAdapter.disable();
            }
        }
    }// blOnOffOnClick()
    ...
    public void pickFileOnClick() {
        Fragment frag = getSupportFragmentManager().findFragmentById(R.id.fragment_container);
        if(frag!=null && (frag instanceof BlConnectionFragment)) {
            if (((BlConnectionFragment) frag).getMaxAllowedTracks() != "")
    ...
    public void onPressRelease(boolean b, int keyIndex){
        if(appStatus.statusCode==MyConstants.STATE_CONNECTED){
            ...
            byte onOffCmd = b ? MidiUtils.ON_CMD : MidiUtils.OFF_CMD;
        ...
        switch (requestedCode){
            ...
            case MyConstants.REQ_CODE_FILE_MANAGER:
                if(resultCode==RESULT_OK && data!=null) {
                    ...
                    if (frag != null && (frag instanceof BlConnectionFragment))
                        fileUri = checkFile(((BlConnectionFragment)frag).getMaxAllowedTracks(), uri) ? uri : null;
    ...
    private void sendOnOffCMD(byte onOffCmd, int keyIndex) {
        sendDataBL.write(onOffCmd);
        sendDataBL.write(MidiUtils.KEYBOARD_CODES[keyIndex]);
        sendDataBL.write(MidiUtils.VEL[2]); // Normal vel by default
    }// sendOnOffCMD()
    ...
    private void myPrintMsg(String msg){
    ...
    private class MyAppStatus implements Runnable {
        ...
        public MyAppStatus(String s, int status, int n ){
            ...
            handlerBl = new Handler(new Handler.Callback() {
                ...
                public boolean handleMessage(@NonNull Message msg) {
                    if(msg.what >= MyConstants.STATE_BL_ON && msg.what <= MyConstants.STATE_CONNECTION_FAILED){
        ...
        public void run(){
            try {
                ...
                e.printStackTrace();
                myPrintMsg("Error while executing app status thread");
            }
        ...
        private void updateBl(String s, int statCode){
            Fragment frag = getSupportFragmentManager().findFragmentById(R.id.fragment_container);
            updateFlag = ( frag == null || !(frag instanceof BlConnectionFragment || frag instanceof KeyboardFragment) );
            if(frag != null ){
    ...
    private class MyBlClient implements Runnable {
        ...
        public MyBlClient(BluetoothDevice targetDevice){
            ...
            try{ socket = targetDevice.createRfcommSocketToServiceRecord(MyConstants.MY_UUID); }
                ...
                e.printStackTrace();
                myPrintMsg("Fail turning on client thread");
            }
        }
        ...
        public void run(){
            try {
                ...
                sendDataBL = new SendDataBL(socket);
                Thread th = new Thread(sendDataBL);
                th.start();
                try {
                    while(!kill)
                        Thread.sleep(MyConstants.THREAD_SLEEP_TIME);
                    ...
                    e.printStackTrace();
                    myPrintMsg("Error while executing client thread");
                }
            }
            catch (IOException e){
                e.printStackTrace();
    ...
    private class SendDataBL implements Runnable {
        private OutputStream outputStream;
        private boolean kill;

        public SendDataBL(BluetoothSocket socket){
            kill = false;
            try { outputStream = socket.getOutputStream(); }
            catch (IOException e){
                e.printStackTrace();
                myPrintMsg("Error while turning on send data thread");
            }
        }
        ...
        public void run(){
            ...
            try {
                ...
                e.printStackTrace();
                myPrintMsg("Error while executing send data thread");
            }
        }
        ...
        public void write(byte b){
            try{ outputStream.write(b); }
                ...
                e.printStackTrace();
                myPrintMsg("Error while sending data bytes");
            }
        }
